...
Complete machine kickstart with integrated router VM build logic
...
check_prerequisites() {
    log "=== KICKSTART V2: Checking Prerequisites ==="
...
step3_build_router_vm() {
    if [[ "$SKIP_ROUTER" == true ]]; then
        ROUTER_SKIP_MSG="⏭️ Router VM: Skipped (--skip-router)"
        return
    fi
...
step5_build_pentest_vm() {
    if [[ "$SKIP_PENTEST" == true ]]; then
        PENTEST_SKIP_MSG="⏭️ Pentest VM: Skipped (--skip-pentest)"
        return
    fi
...
step11_create_setup_guide() {
    ...
    local setup_file="$setup_dir/${MACHINE_NAME}-kickstart-complete.md"
...
---
Generated by machine-kickstart-v2.sh - Splix VM Automation
GUIDEEOF
...
show_completion_summary() {
    log ""
    success " MACHINE KICKSTART V2 COMPLETED! "
    log "======================================="
    log ""
    log "Configuration:"
    log "  Machine: $MACHINE_NAME"
    ...
    log "Complete guide:"
    log "  cat generated/setup-examples/${MACHINE_NAME}-kickstart-complete.md"
    log ""
...
main() {
    log ""
    log "🚀 SPLIX MACHINE KICKSTART V2 🚀"
    log "================================"
    log ""
    ...
    log ""
    success "✅ Machine kickstart v2 completed successfully!"
}
